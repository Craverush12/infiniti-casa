...
export const mockUseAuth = () => {
  ...
  const generateOTP = async (phone: string) => {
    try {
      setError(null);
      const result = await MockAuthService.generateOTP(phone);
  ...
  const resendOTP = async (phone: string) => {
    try {
      setError(null);
      const result = await MockAuthService.resendOTP(phone);
  ...
  const updateProfile = async (updates: Partial<UserProfile>) => {
    try {
      setError(null);
      const updatedProfile = await MockAuthService.updateProfile(updates);
  ...
  const updatePreferences = async (preferences: Record<string, any>) => {
    try {
      setError(null);
      const success = await MockAuthService.updatePreferences(preferences);
  ...
  const isPhoneVerified = async (phone: string) => {
    try {
      return await MockAuthService.isPhoneVerified(phone);
    } catch (error) {
